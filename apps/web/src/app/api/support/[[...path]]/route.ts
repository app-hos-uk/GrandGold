...
/**
 * Proxy /api/support and /api/support/* to order-service so the client's
 * Authorization header is forwarded (Next.js rewrites do not forward headers).
 * Required for creating tickets and listing tickets (authenticate + authorize).
 */
...
async function proxy(request: NextRequest, path: string) {
  const url = `${ORDER_SERVICE_URL}/api/support${path}${request.nextUrl.search}`;
