...
export function validatePhone(phone: string, country: 'IN' | 'AE' | 'UK'): boolean {
  const schema = phoneSchemaByCountry[country];
  return schema.safeParse(phone).success;
}

export function validatePostalCode(postalCode: string, country: 'IN' | 'AE' | 'UK'): boolean {
  const config = COUNTRY_CONFIGS[country];
  return config.addressFormat.postalCodePattern.test(postalCode);
}

// Tax ID validation by country
export function validateTaxId(taxId: string, country: 'IN' | 'AE' | 'UK'): boolean {
